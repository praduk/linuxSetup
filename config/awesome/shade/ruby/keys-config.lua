...
-- change window focus by direction (global)
local focus_switch_gbyd = function(dir)
	return function()
...
-- swap window by direction (global)
local focus_swap_gbyd = function(dir)
	return function()
...
function hotkeys:init(args)
	...
	self.raw.root = {
		...
		{
			{ env.mod }, ";", function () redflat.service.navigator:run() end,
			{ description = "[Hold] Tiling window control mode", group = "Window control" }
		},
		{
		...
		{
			{ env.mod }, "Return", function() awful.spawn("x-terminal-emulator") end,
			{ description = "Open a terminal", group = "Applications" }
		},
		{
			{ env.mod }, "Print", function() awful.spawn(os.getenv("HOME") .. "/.config/awesome/startup.sh") end,
			{ description = "Run Startup Script", group = "Applications" }
		},
		{
		...
		{
			{ "Control", "Shift" }, "f", function() awful.spawn("google-chrome") end,
			{ description = "Open Chrome", group = "Applications" }
		},
		{
